...
interface ComponentConfig {
  attr?: Array<{
    components?: Array<{
      props?: ComponentConfigItem[]
      attr?: ComponentConfigItem[]
      [key: string]: any
    }>
    [key: string]: any
  }>
...
export default {
  ...
  // 计算画布模式样式
  calculationCanvasModeStyle(currentModeItem: DeviceModeItem): { [key: string]: string } {
   ...
   * @returns {any} - 处理后的新数据。
   */
  generateDataId(data: any, idGenerator?: () => string): any {
   ...
   */
  processComponentAttrConfig(config: ComponentConfig, dict: { [key: string]: (item: any) => any }): ComponentConfig {
    // --- 内部辅助函数，用于处理核心的转换逻辑 ---
    /**
     * 接收一个项目数组，使用字典进行转换。
     ...
     */
    const transformArray = (itemsArray: ComponentConfigItem[], extraData: any = {}): ComponentConfigItem[] => {
      // 步骤1：健壮性检查，确保输入是数组。
   ...
   *   函数签名：(key, value) => any
   */
  exportToFile(data: any, filename: string = 'export.txt', options: ExportOptions = {}): void {
    // --- 1. 合并默认配置与用户自定义配置 ---
     ...
     * @returns {any} - 转换后的值。
     */
    const smartJsonReplacer = (key: string, value: any): any => {
     ...
     */
    const convertToCsv = (arr: any[], headers?: string[]): string => {
    ...
    const fileType = config.type || filename.split('.').pop()?.toLowerCase() || 'txt'
  ...
  formatJSONLikeImage(array: any[]): string {
