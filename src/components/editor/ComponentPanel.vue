  ...
  <div class="flex flex-col">
    ...
    <div class="overflow-y-auto h-[650px]">
      <div class="flex flex-col gap-1 p-2">
        <!-- 遍历每个UI库 -->
        <div v-for="library in libraries" :key="library.key" class="library-group">
          <!-- UI库标题 -->
          <button
          ...
          >
            ...
            <span
              ...
              :class="{ 'rotate-180': expandedLibraries.includes(library.key) }"
            >
              expand_more
            </span>
          </button>
          ...
          <!-- 分类列表（展开时显示） -->
          <div v-if="expandedLibraries.includes(library.key)" class="ml-4 mt-1 space-y-1">
            <!-- 遍历该库下的每个分类 -->
            <div v-for="category in library.categories" :key="category.key">
              <!-- 分类标题 -->
              <button
              ...
              >
                ...
                <span
                  class="material-symbols-outlined text-lg transition-transform"
                  :class="{ 'rotate-180': expandedCategories[`${library.key}-${category.key}`] }"
                >
                  {{
                    expandedCategories[`${library.key}-${category.key}`]
                      ? 'expand_less'
                      : 'expand_more'
                  }}
                </span>
