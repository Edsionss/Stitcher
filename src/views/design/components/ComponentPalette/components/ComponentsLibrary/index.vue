      ...
      <div v-for="framework in frameworkLibrary" :key="framework.FrameworkName" class="framework-panel__item">
        ...
        <div class="framework-panel__content" v-if="activeFrameworkName === framework.FrameworkName">
          <van-tabs color="#409EFF" v-model="activeComponents">
            <van-tab v-for="(components, index) in framework.ComponentLibrary" :title="components.componentsLabel"
              :name="components.componentsType" :key="components.componentsType + index">
                ...
                <div class="group-list__item" v-for="group in components.group" :key="group.name">
                  <div class="group-list__header" @click="foldItem(framework.FrameworkName, group.groupName)">
                    ...
                    <i :class="['group-list__icon', 'el-icon']">
                      <component :is="computedFoldState(groupShowStates[framework.FrameworkName + group.groupName], true)
                        "></component>
                    </i>
                  </div>
                  <div class="core-panel__item-content" v-show="isGroupOpen(framework.FrameworkName, group.groupName)">
                    <VueDraggable ref="el" v-model="group.groupComponents">
                      <transition-group tag="div" class="drag-zone__group">
                        <div class="drag-zone__item" v-for="(component, index) in group.groupComponents"
...
const frameworkLibrary = reactive<FrameworkItem[]>(FrameworkLibrary),
  activeFrameworkName = ref<string>(''),
  activeComponents = ref<string>('base'),
  groupShowStates = reactive<Record<string, boolean>>({})

const computedFoldState = (activeFrameworkName: string, frameworkName: string) => {
...
const foldItem = (frameworkName: string, groupName: string) => {
  //content
  groupShowStates[frameworkName + groupName] = !groupShowStates[frameworkName + groupName]
}

const isGroupOpen = (frameworkName: string, groupName: string) => {
  //content
