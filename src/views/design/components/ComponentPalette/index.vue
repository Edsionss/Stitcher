...
const Tools = readonly<ToolItem[]>([{ name: 'close', tips: '关闭', icon: 'CloseBold' }])

const currentModule = reactive<ModuleItem>(_.cloneDeep(Modules[DefaultModule])),
  panelShow = ref<boolean>(true),
  searchVal = ref<string>(''),
  searchType = ref<string>('')

const changeModulePanel = (item: ModuleItem, index: number) => {
...
const handlerTool = (item: ToolItem, index?: number) => {
  const { name } = item
  switch (name) {
    case 'close':
